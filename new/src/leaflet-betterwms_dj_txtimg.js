...
/*    $.ajax({
  ...
  showGetFeatureInfoJson: function (latlng, data) {
    ...
    data.features.forEach(function(item){  //$.each(data.features, function (i, item) {
        ...
        + "<div style='position: relative;'><table style='background: no-repeat 15px 75%/30px url(\"icons/"+item.properties.EC_GROUP.replace(/;.*/,'')+".png\"); border-spacing:0px'"  //, no-repeat left/contain url(\"icons/warn.png\"), linear-gradient(to right, "+color[item.properties.SEVERITY]+" 54px,transparent 54px)
