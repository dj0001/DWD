        ...
        fetch("https://nominatim.openstreetmap.org/search?q=" + q + "&format=json&limit=1")
          .then(function(response) {
            response.json().then(function(data) {var e = [data[0].lat,data[0].lon];
              karte.setView(e);
              if(confirm("F\u00FCgehinzu Marker (oder Ersetze)?")) addmarker(e); else warnlayer.getFeatureInfoJsonp({latlng: e});  //warnlayer.getFeatureInfoJsonp({latlng: e})
...
function Timer(layer) {  //add locations
this.lay=layer;
this.tID=0; this.t2=0;
this.lay._marker.on('move', function(e){clearTimeout(this.tID); this.t2=Date.now(); this.tID=setTimeout(this.updatem,auto,e,this)}, this);
this.updatem = function (e,that) {that.lay.getFeatureInfoJsonp({latlng:e.latlng})}
}

var lnr=0; function addmarker(latlon){
var layer=window["l"+lnr]
var layer = L.tileLayer.betterWms("",warnlayer.options)
layer._marker = L.marker(latlon).addTo(karte)
layer.getFeatureInfoJsonp({latlng:layer._marker.getLatLng()})
new Timer(layer); lnr++;
}
