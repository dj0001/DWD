...
/*    $.ajax({
  ...
  showGetFeatureInfoJson: function (latlng, data) {
    ...
    var severity=["Minor","Moderate","Severe","Extreme"]  //
    var icons=["WIND","SNOWFALL","THUNDERSTORM","FOG","HEAT","FROST","kueste","wald","lawinen","RAIN"]; //,"tornado?","hochwasser","regenh"
    data.features.sort(function(a, b){return new Date(a.properties.ONSET) - new Date(b.properties.ONSET)});  //sort array
    data.features.forEach(function(item){ //console.log(severity.indexOf(item.properties.SEVERITY)+2)  //$.each(data.features, function (i, item) {
        ...
        //+ "<div style='position: relative;'><table style='background: no-repeat 12px 75%/32px url(\"icons/"+item.properties.EC_GROUP.replace(/;.*/,'')+".png\"); border-spacing:0px'"  //, no-repeat left/contain url(\"icons/warn.png\"), linear-gradient(to right, "+color[item.properties.SEVERITY]+" 54px,transparent 54px)
        + "<div style='position: relative;'><table style='background: no-repeat left/contain url(\"http://www.meteoalarm.eu/theme/common/pictures/aw"+(icons.indexOf(item.properties.EC_GROUP.replace(/;.*/,''))+1)+(severity.indexOf(item.properties.SEVERITY)+2)+".jpg\"); border-spacing:0px'"
